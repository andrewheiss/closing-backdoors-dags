...
> See the [actual blog post](https://www.andrewheiss.com/blog/2020/02/25/closing-backdoors-dags/).
...
- Felix Elwert, "Graphical Causal Models," chap. 13 in *Handbook of Causal Analysis for Social Research*, ed. Stephen L. Morgan (New York: Springer, 2013), 245–273, doi: [10.1007/978-94-007-6094-3_13](https://doi.org/10.1007/978-94-007-6094-3_13)
...
- [Incorrect "correlation is not causation" estimate](#incorrect-%22correlation-is-not-causation%22-estimate)
...
```{r load-libraries-make-dag, warning=FALSE, message=FALSE, fig.width=7, fig.height=4.25}
...
```{r plot-distributions-wrong, fig.width=7, fig.height=4.25}
...
```{r gre-math-camp, fig.width=7, fig.height=4.25}
...
```{r simpler-dag, fig.width=7, fig.height=4.25}
...
```{r match-stuff, warning=FALSE, message=FALSE}
...
One potential downside to matching is that it throws away data. Notice how there are only `r nrow(math_camp_matched) %>% scales::comma()` rows in the `math_camp_matched` dataset, while using inverse probability weights let us use the full `r nrow(math_camp_propensities) %>% scales::comma()` rows in the original data.
...
```{r do-calculus-tiny-dag, fig.width=5, fig.height=3, out.height="80%"}
...
```{r do-calculus-binary, eval=FALSE}
# P(y|do(x=1)) = P(y|x=1, z=1)*P(z=1) + P(y|x=1, z=0)*P(z=0)
mean(y[x == 1 & z == 1]) * mean(z == 1) + mean(y[x == 1 & z == 0]) * mean(z == 0)
```

There are fancy algorithms that can determine the exact adjustmnet formula for a given DAG, and the **causaleffect** package lets you use these algorithms in R. Here's the *do*-calculus version of our math camp example. Unfortunately we have to rewrite the DAG with a different syntax for it to work:

```{r math-dag-do-calculus, warning=FALSE, message=FALSE}
...
```{r compare-everything, warning=FALSE, message=FALSE, fig.width=7, fig.height=4.25}
library(ggstance)
